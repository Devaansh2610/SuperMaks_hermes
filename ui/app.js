...
/* ══════════════ 10b. the wake phrase ══════════════
   Once a day, the HUD boots into "dormant" — near-black, a slow breathing
   mark, nothing else — and runs a background speech recognizer listening only
   for the wake phrase. Say it, and the daily briefing runs. After that, wake-
   phrase listening turns off until the calendar date changes; every other
   activation that day goes through push-to-talk (long-press Control) or the
   Voice button instead.

   This deliberately never touches Fish Audio — streaming the mic to a paid
   STT endpoint all day just to catch one phrase would be both slow and not
   free. The browser's own recognizer is free and already running locally. */

const WAKE_PHRASE = /\bwake\s*up\b[\s\S]{0,24}\bdad(?:dy)?'?s?\b[\s\S]{0,24}\bhome\b/i;
const WAKE_STORE_KEY = 'supermaks.lastWakeDate';

let dormantOn = false, wakeActive = false, wakeRecognition = null;

const today = () => new Date().toDateString();
function alreadyWokenToday(){
  try { return localStorage.getItem(WAKE_STORE_KEY) === today(); } catch(_){ return false; }
}
function markWoken(){
  try { localStorage.setItem(WAKE_STORE_KEY, today()); } catch(_){}
}
...
// If the tab is left open across midnight, re-arm without needing a reload.
setInterval(() => {
  if (!dormantOn && !running && !convo && !pttArmed && !alreadyWokenToday()) enterDormant();
}, 5 * 60 * 1000);
...
document.addEventListener('keydown', e => {
  ...
  if (dormantOn || running || convo || pttArmed || pttTimer) return;
  pttTimer = setTimeout(startPTT, PTT_HOLD_MS);
});
...
async function startPTT(){
  pttTimer = null;
  if (dormantOn || running || convo) return;
  micButton(true, 'Listening');
  setState('listening','listening…');
...
(async () => {
  ...
  if (alreadyWokenToday()) setState('standby','awaiting uplink');
  else enterDormant();
